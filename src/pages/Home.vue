...
const fetchFavorites = async () => {
  try {
    const { data: favorites } = await axios.get('https://cdfdd4035ec5d396.mokky.dev/favorites');
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get('https://cdfdd4035ec5d396.mokky.dev/items', {
      params
    });
...
<template>
  <div class="flex justify-between items-center flex-wrap gap-5">
    <h2 class="text-3xl font-bold w-full">Магазин</h2>
